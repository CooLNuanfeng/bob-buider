...
module.exports = function(source,opts){
    ...
    fs.writeFile(path.resolve(opts.story,opts.name,'package.json'),JSON.stringify(pkg),function(err){
        if(err) console.log(err);
        console.log(`✨`,chalk.rgb(57, 235, 237)(' install package ......\n'));
        console.log(`🚀`, ' please wait patiently.\n');

        Promise.all([
            execa.shell('npm i '+devPkg.join(' ')+' --save-dev',{'cwd':path.resolve(opts.story,opts.name)}).then(()=>console.log(chalk.gray('devDependencies is ok'))),
            execa.shell('npm i '+dependPkg.join(' ')+' --save',{'cwd':path.resolve(opts.story,opts.name)}).then(()=>console.log(chalk.gray('dependencies is ok')))
        ]).then(()=>{
